...
# Возвращается конкатенация отсортированных подмассивов less, опорного элемента pivot и отсортированного подмассива
# greater.
...
def Partition(values: list, l, r):
    # разделение массива на подмассивы с элементами, меньшими и большими опорного элемента
    x = values[r]
    less = l
...
################################################################
# ___алгоритм бинарного поиска
# Алгоритм бинарного поиска предполагает, что список или массив, в котором будет происходить поиск, отсортирован по
# возрастанию. Это требование является необходимым для правильной работы алгоритма.Интуитивно понятно, что если список
# не отсортирован, то нам придется просматривать все элементы, чтобы быть уверенными, что искомого элемента в нем нет.
# В этом случае бинарный поиск не даст нам преимущества перед обычным линейным поиском.В алгоритме бинарного поиска
# применяется стратегия "разделяй и властвуй". Мы сравниваем искомый элемент с элементом в середине списка.
# Если искомый элемент меньше, чем средний элемент, то мы сужаем диапазон поиска до левой половины списка. Если
# искомый элемент больше, чем средний элемент, то сужаем диапазон до правой половины списка. Продолжаем делить
# диапазон на две половины и сравнивать искомый элемент с элементом в середине до тех пор, пока не найдем искомый
# элемент или не убедимся, что он отсутствует. Однако, если список не отсортирован, то мы не можем безопасно считать,
# что средний элемент действительно разделяет список на две половины. Это приведет к неправильным результатам,
# поскольку элемент, который мы ищем, может оказаться в другой половине списка, что нарушает предположения, на
# которых основан алгоритм бинарного поиска. Поэтому перед использованием алгоритма бинарного поиска необходимо
# убедиться, что список отсортирован по возрастанию, чтобы гарантировать правильные результаты.
...
# 2. через рекурсию
def binarysearch_recur(arr, x, left, right):
    if right >= left:
...
x = 7
print(binarysearch_recur(arr, x, left=0, right=len(arr) - 1))  # indx
